...
// 唯一ID格式验证（6-8位，字母数字下划线）
const UNIQUE_ID_PATTERN = /^[A-Za-z0-9_]{6,8}$/

/**
 * 检查唯一ID是否可用
 */
router.post('/check-unique-id', authMiddleware, (req, res) => {
  try {
    const { uniqueId } = req.body
    const userId = req.user.id

    if (!uniqueId || !uniqueId.trim()) {
      return res.status(400).json({ error: '请输入唯一ID' })
    }

    const trimmedId = uniqueId.trim()

    if (!UNIQUE_ID_PATTERN.test(trimmedId)) {
      return res.status(400).json({ error: '唯一ID需为6-8位字母、数字或下划线组合' })
    }

    const available = Friend.isUniqueIdAvailable(trimmedId, userId)
    ...
    console.error('检查唯一ID错误:', error)
    res.status(500).json({ error: '检查唯一ID失败' })
  }
})
 ...
 * 设置唯一ID
 */
router.post('/set-unique-id', authMiddleware, (req, res) => {
  try {
    const { uniqueId } = req.body
    const userId = req.user.id
    ...
    // 检查是否可用
    if (!Friend.isUniqueIdAvailable(trimmedId, userId)) {
      return res.status(400).json({ error: '该唯一ID已被使用' })
    }

    Friend.setUniqueId(userId, trimmedId)
    ...
    console.error('设置唯一ID错误:', error)
    res.status(500).json({ error: '设置唯一ID失败' })
  }
})
