...
function buildLatestRevisorPrompt({ verb, conjugation, originalQuestion, validatorResult, fixedHint }) {
...
class QuestionRevisorService {
  static async reviseQuestion({ verb, conjugation, originalQuestion, validatorResult, fixedHint }) {
    const settings = getStageSettings('REVISOR')
    const prompt = buildLatestRevisorPrompt({
      verb,
      conjugation,
      originalQuestion,
      validatorResult,
    ...
    console.log(
      `[AI生成][revisor] model=${getConfig().model} prompt=embedded_latest maxTokens=${settings.maxTokens} temperature=${settings.temperature}`
    )
...
module.exports = QuestionRevisorService
